...
for year in range(2014,2033):
    # cut data bw january 2014 and december 2021
    beg_idx = np.where((USA_florida['Year']==year) & (USA_florida['Month']==1))[0][0]
    end_idx = np.where((USA_florida['Year']==(year+7)) & (USA_florida['Month']==12))[0][0]

    reframed_year = reframed.iloc[beg_idx:end_idx+1,:]
    print(reframed_year.shape)
    reframed_year = reframed_year.fillna(0)#remplacer nan des cases per month par des 0
    print(sum(np.array(pd.isna(reframed_year)))) 
    ...
    # USA_florida.loc[USA_florida['Year'] == year+7,['Cases_per_month']] = yhat
    reframed.iloc[end_idx-13:end_idx-1,-1] = yhat
...
A = range(2021,2040)
plt.xticks(range(0,19),['2021','2022','2023','2024','2025','2026','2027','2028','2029','2030','2031','2032','2033','2034','2035','2036','2037','2038','2039'],rotation = 45)
# plt.ylim([0,2])
# plt.xlim([-1,370])
plt.legend()
plt.show()
